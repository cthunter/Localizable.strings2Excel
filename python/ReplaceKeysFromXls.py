...
def addParser():
    ...
    parser.add_option("-s", "--sourceFloderPath",
                      help="Source Floder Path.",
                      metavar="sourceFloderPath")
...
def startConvert(options):
    ...
    sourceFloderPath = options.sourceFloderPath
    ...
    if filePath is not None:
        ...
        replaceLocalizableFile(table,targetFloderPath,iOSAdditional,androidAdditional)
...
def replaceLocalizableFile(table,targetFloderPath,iOSAdditional,androidAdditional):
    ...
    for index in range(len(firstRow)):
        if index > 0:
            ...
            del values[0]
            # iOS
            LocalizableStringsFileUtil.fuzzyReplaceLocalizableFile(keys,values,targetFloderPath + "/ios/"+languageName+".lproj/",iOSAdditional)
